...
float sdMetaBalls(vec3 pos) // adapted from https://www.shadertoy.com/view/ld2GRz
{
  ...
  {
    vec3 center = 1.0 * vec3(sin(10 * i + iTime), cos(20 * i + iTime), sin(30 * i + iTime));
...
float map(in vec3 pos)
{
  ...
  // rotating box
  d = min(d, box(rotateY(scale(pos, vec3(3, 3, 3)), iTime), vec3(.1, 1, 1)));

  // multi-balls
  for (int i = 0; i < 4; ++i)
  {
    vec3 p = translate(pos, 1.0 * vec3(sin(10 * i + iTime), cos(20 * i + iTime), sin(30 * i + iTime)));
    //d = min( d, sphere(p, 2) );
  }
...
{
  //vec3 rayOrigin = vec3(sin(iTime), 3*cos(0.4*iTime), 10); // ray origin

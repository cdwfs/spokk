...
PillarsApp::PillarsApp(Application::CreateInfo &ci) :
  ...
  for(uint32_t pframe = 0; pframe < PFRAME_COUNT; ++pframe) { 
    ...
    dset_writer.BindTexelBuffer(visible_cells_buffer_.View(pframe),
      pillar_vs_.GetDescriptorBindPoint("visible_cells").binding);
    dset_writer.BindTexelBuffer(heightfield_buffer_.View(pframe),
      pillar_vs_.GetDescriptorBindPoint("cell_heights").binding);

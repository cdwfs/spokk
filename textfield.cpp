...
uint16_t F32toU16N(float f) {
  return (uint16_t)( my_clamp(f, 0.0f, 1.0f) * (float)UINT16_MAX + 0.5f );
}
int16_t F32toS16(float f) {
  return (int16_t)my_clamp(f, (float)INT16_MIN, (float)INT16_MAX);
}
...
class TextfieldApp : public spokk::Application {
public:
  explicit TextfieldApp(Application::CreateInfo &ci);
  virtual ~TextfieldApp();
  TextfieldApp(const TextfieldApp&) = delete;
  const TextfieldApp& operator=(const TextfieldApp&) = delete;
...
TextfieldApp::TextfieldApp(Application::CreateInfo &ci) :
    Application(ci) {
  ...
  string_info.str = "This is a long string, just to test out string rendering. It includes one longer-than-average-string-to-stress-test the wrapping code.";
  ...
  atlas_image_ci.usage = VK_IMAGE_USAGE_SAMPLED_BIT |
  ...
  int atlas_load_err = font_atlas_image_.LoadSubresourceFromMemory(device_context_, blitter_, graphics_and_present_queue_,
    atlas_image_pixels.data(), atlas_ci.image_width, atlas_ci.image_height, dst_subresource);
  ...
  int mipmap_gen_err = font_atlas_image_.GenerateMipmaps(device_context_, graphics_and_present_queue_, mipmap_barrier, 0, 0);
...
void TextfieldApp::Update(double dt) {
...
void TextfieldApp::HandleWindowResize(VkExtent2D new_window_extent) {
...
void TextfieldApp::CreateRenderBuffers(VkExtent2D extent) {
  // Create depth buffer

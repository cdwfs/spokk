...
//
Font::Font()
  : ascent_(0), descent_(0), line_gap_(0) {
}
Font::~Font() {
}
...
void Font::ComputeStringBitmapDimensions(const StringRenderInfo& info,
...
int Font::RenderStringToBitmap(const StringRenderInfo& info,
    uint32_t bitmap_w, uint32_t bitmap_h, uint8_t *bitmap_pixels) const {
  ...
  for(int ch = 0; ch < str_length; ++ch) {
    ...
    stbtt_GetGlyphBitmapBoxSubpixel(&font_info_, glyphs[ch].glyph_index,
      scale, scale, glyphs[ch].x_shift, glyphs[ch].y_shift,
      &glyphs[ch].x0, &glyphs[ch].y0, &glyphs[ch].x1, &glyphs[ch].y1);
    ZOMBO_ASSERT(uint32_t(glyphs[ch].x1) < bitmap_w, "x1 (%d) >= bitmap_w (%d)", glyphs[ch].x1, bitmap_w);
    ZOMBO_ASSERT(uint32_t(baseline + glyphs[ch].y1) < bitmap_h, "y1 (%d) >= bitmap_h (%d)", glyphs[ch].y1, bitmap_h);
  ...
  for(int ch = 0; info.str[ch] != '\0'; ++ch) {
    ...
    stbtt_MakeGlyphBitmapSubpixel(&font_info_, glyph_pixels.data(),
      glyphs[ch].x1 - glyphs[ch].x0, glyphs[ch].y1 - glyphs[ch].y0,
      max_glyph_width, scale, scale, glyphs[ch].x_shift, glyphs[ch].y_shift,
      glyphs[ch].glyph_index);
    int si = 0, di = 0;
    uint8_t *dp = &bitmap_pixels[(int(glyphs[ch].ypos) + baseline + glyphs[ch].y0)*bitmap_w + int(glyphs[ch].xpos) + glyphs[ch].x0];
    ...
    for(int sy=0; sy<glyph_h; ++sy) {
      ...
      for(int sx=0; sx<glyph_w; ++sx) {
        ZOMBO_ASSERT(dp + di <= bitmap_pixels + (bitmap_w*bitmap_h),
  ...
  ZOMBO_ASSERT(write_success, "String image write failure: %d", write_success);
#endif
...
void Font::ComputeGlyphInfoAndBitmapDimensions(const StringRenderInfo& info,
    size_t str_length, GlyphInfo *out_glyphs, uint32_t *out_w, uint32_t *out_h) const {
  ZOMBO_ASSERT(info.x_max > info.x_min, "x_max (%d) must be greater than x_min (%d)",
    info.x_max, info.x_min);
  ...
  for(int ch = 0; ch < str_length; ++ch) {
    ...
    if (info.str[ch+1] != '\0') {
      ...
      xpos += scale * stbtt_GetGlyphKernAdvance(&font_info_, out_glyphs[ch].glyph_index,
        out_glyphs[ch+1].glyph_index);
    }
...
int FontAtlas::Create(const FontAtlasCreateInfo& ci, uint8_t* out_bitmap) {
  ...
  int err = stbtt_PackBegin(&pack_context, out_bitmap, image_width_, image_height_,
    bitmap_row_nbytes, padding, alloc_context);
  if (err != 1) {
    return -3;  // stbtt_PackBegin() error
  }
  ZOMBO_ASSERT(err == 1, "stbtt_PackBegin() error: %d", err);
  ...
  ZOMBO_ASSERT(write_success, "Font image write failure: %d", write_success);
#endif
  return 0;
}
...
int FontAtlas::GetStringQuads(const char* str, size_t str_len, Quad* out_quads) const {
  ...
  for(size_t i=0; i<str_len; ++i) {
    ...
    stbtt_GetPackedQuad(const_cast<stbtt_packedchar*>(char_data_.data()),
      image_width_, image_height_, codepoint - codepoint_first_,
      &pos_x, &pos_y, &quads[i], align_to_integer);
    if (str[i] == ' ') {

...
void RenderPass::InitFromPreset(Preset preset, VkFormat output_color_format) {
  if (preset == Preset::COLOR) {
    ...
    subpass_dependencies[0].dstStageMask = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
    ...
    subpass_dependencies[1].dstStageMask = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
...
VkResult RenderPass::Finalize(const Device& device, VkPipelineBindPoint bind_point, VkSubpassDescriptionFlags flags) {
  ...
  VkResult create_result = vkCreateRenderPass(device, &render_pass_ci, device.HostAllocator(), &handle);
...
void RenderPass::Destroy(const Device& device) {
  if (handle != VK_NULL_HANDLE) {
    vkDestroyRenderPass(device, handle, device.HostAllocator());

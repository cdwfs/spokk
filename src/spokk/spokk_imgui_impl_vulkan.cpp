...
struct ImGui_ImplVulkan_Data
{
    ...
    VkDescriptorPool            DescriptorPool; // spokk: create/manage descriptor pool internally
    VkPipeline                  Pipeline;
    uint32_t                    Subpass;
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!bd->DescriptorPool)
    {
      ...
      err = vkCreateDescriptorPool(v->Device, &info, v->Allocator, &bd->DescriptorPool);
      check_vk_result(err);
    }
    ...
    // Create Descriptor Set:
    {
        ...
        alloc_info.descriptorPool = bd->DescriptorPool; // spokk: descriptor pool is managed internally
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    if (bd->DescriptorPool)       { vkDestroyDescriptorPool(v->Device, bd->DescriptorPool, v->Allocator); bd->DescriptorPool = VK_NULL_HANDLE; } // spokk: descriptor pool is managed internally

...
static uint32_t                 g_Subpass = 0; // spokk: add subpass field
static VkDeviceSize             g_BufferMemoryAlignment = 256;
static VkPipelineCreateFlags    g_PipelineCreateFlags = 0x00;
static VkDescriptorSetLayout    g_DescriptorSetLayout = VK_NULL_HANDLE;
static VkDescriptorPool         g_DescriptorPool = VK_NULL_HANDLE; // spokk: create/manage descriptor pool internally
...
static void ImGui_ImplVulkan_CreateShaderModules(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ...
    if (g_ShaderModuleVert == VK_NULL_HANDLE)
    {
    ...
    if (g_ShaderModuleFrag == VK_NULL_HANDLE)
    {
...
static void ImGui_ImplVulkan_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator, VkPipelineCache pipelineCache, VkRenderPass renderPass, uint32_t subpass, VkSampleCountFlagBits MSAASamples, VkPipeline* pipeline) // spokk: added subpass
{
    ...
    info.subpass = subpass; // spokk: add subpass
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!g_DescriptorPool)
    {
      ...
      info.maxSets = v->ImageCount;
      info.poolSizeCount = sizeof(pool_sizes) / sizeof(pool_sizes[0]);
      info.pPoolSizes = pool_sizes;
      err = vkCreateDescriptorPool(v->Device, &info, v->Allocator, &g_DescriptorPool);
      check_vk_result(err);
    }
    ...
    // Create Descriptor Set:
    {
        ...
        alloc_info.descriptorPool = g_DescriptorPool; // spokk: descriptor pool is managed internally
    ...
    ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, v->PipelineCache, g_RenderPass, g_Subpass, v->MSAASamples, &g_Pipeline); // spokk: added subpass
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    if (g_DescriptorPool)       { vkDestroyDescriptorPool(v->Device, g_DescriptorPool, v->Allocator); g_DescriptorPool = VK_NULL_HANDLE; } // spokk: descriptor pool is managed internally
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass, uint32_t subpass) // spokk: add subpass index
{
    ...
    // IM_ASSERT(info->DescriptorPool != VK_NULL_HANDLE); // spokk: descriptor pool is created internally

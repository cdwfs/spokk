...
static ImageFileDataFormat DdsParsePixelFormat(const DdsPixelFormat pf) {
  ...
  if (pf.flags & PF_FLAGS_RGBA) {
    ...
    case 8:
      if (DdsIsPfMask(pf, 0x000000ff, 0x00000000, 0x00000000, 0x00000000)) return IMAGE_FILE_DATA_FORMAT_R8_UNORM;
      break;
    }
...
static int LoadImageFromDds(ImageFile *out_image, const char *image_path) {
  ...
  if (!dds_file) return -3;  // Couldn't open file for reading
  ...
  if ((header->flags & (HEADER_FLAGS_WIDTH | HEADER_FLAGS_HEIGHT)) != (HEADER_FLAGS_WIDTH | HEADER_FLAGS_HEIGHT)) {
    // technically DDSD_CAPS and DDSD_PIXELFORMAT are required as well, but their absence is so widespread that they
  ...
  if ((header->caps & SURFACE_FLAGS_COMPLEX) && (header->caps2 & CUBEMAP_FLAG_ISCUBEMAP)) {
    // clang-format off
    const uint32_t kCubemapFlagAllFaces = CUBEMAP_FLAG_ISCUBEMAP |
  ...
  if ((header->flags & HEADER_FLAGS_DEPTH) &&
      (header->caps2 &
          CUBEMAP_FLAG_VOLUME))  // (header->dwCaps & SURFACE_FLAGS_COMPLEX) -- doesn't always seem to be set?
  {
    if (header->depth == 0) {
      free(dds_bytes);
  ...
  out_image->row_pitch_bytes = is_compressed ? (bytes_per_texel_block * IMAGEFILE__MAX(1U, (header->width + 3U) / 4U))
...
static int LoadImageFromAstc(ImageFile *out_image, const char *image_path) {
  ...
  if (!astc_file) return -2;  // Couldn't open file for reading
  ...
  fclose(astc_file);
  if (read_size != 1) return -4;  // Couldn't read file contents
...
#define ELSE_IF_BLOCKDIM_THEN_SET_DATA_FORMAT(dimx, dimy)                               \
  ...
  out_image->data_format = is_srgb ? IMAGE_FILE_DATA_FORMAT_ASTC_##dimx##x##dimy##_SRGB \
                                   : IMAGE_FILE_DATA_FORMAT_ASTC_##dimx##x##dimy##_UNORM

  if (header->blockdim_x == 0 && header->blockdim_y == 0) out_image->data_format = IMAGE_FILE_DATA_FORMAT_UNKNOWN;
...
static uint16_t byte_swap_u16(uint16_t in) { return ((in & 0xFF) << 8) | ((in & 0xFF00) >> 8); }
...
static int LoadImageFromKtx(ImageFile *out_image, const char *image_path) {
  ...
  if (!ktx_file) return -3;  // Couldn't open file for reading
  ...
  if (header->endianness == 0x01020304) {
    ...
    for (uint32_t iMip = 0; iMip < header->numberOfMipmapLevels; ++iMip) {
      // TODO(https://github.com/cdwfs/spokk/issues/4): confirm both cubemap and cubemap array files are handled
      // correctly here; they are paded differently.
  ...
  out_image->mip_levels =
      IMAGEFILE__MAX(1, header->numberOfMipmapLevels);  // 0 is a request to generate the full mip chain.
  ...
  out_image->flags = 0;
  if (header->numberOfFaces == 6) out_image->flags |= IMAGE_FILE_FLAG_CUBE_BIT;
  return 0;
}
...
int ImageFileCreate(ImageFile *out_image, const char *image_path) {
  ...
  if (suffix == NULL) return -1;  // No filename suffix
...
size_t ImageFileGetSubresourceSize(const ImageFile *image, const ImageFileSubresource subresource) {
  ...
  case IMAGE_FILE_TYPE_DDS: {
    ...
    uint32_t mip_pitch =
        is_compressed ? ((mip_width + 3) / 4) * bytes_per_texel_block : mip_width * bytes_per_texel_block;
...
void *ImageFileGetSubresourceData(const ImageFile *image, const ImageFileSubresource subresource) {
  ...
  case IMAGE_FILE_TYPE_DDS: {
    ...
    intptr_t out_ptr = (intptr_t)image->file_contents + texels_base_offset + subresource.array_layer * layer_size +
